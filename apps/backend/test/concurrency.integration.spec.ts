...
describe('SessionService — concurrency (integration)', () => {
  ...
  it(`allows exactly ${MAX_CONCURRENT} of ${CONCURRENT_REQUESTS} concurrent creates`, async () => {
    ...
    const results = await Promise.allSettled(
      Array.from({ length: CONCURRENT_REQUESTS }, () =>
        service.createSession({ arenaId, endTime, startTime })
      )
    );
  ...
  it('allows boundary-touching sessions (5 end at T, 5 start at T)', async () => {
    ...
    await Promise.all(
      Array.from({ length: 5 }, () =>
        service.createSession({ arenaId, endTime: T, startTime: beforeT })
      )
    );
    ...
    const results = await Promise.allSettled(
      Array.from({ length: 5 }, () =>
        service.createSession({ arenaId, endTime: afterT, startTime: T })
      )
    );
